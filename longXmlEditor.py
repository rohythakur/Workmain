...
parser = argparse.ArgumentParser(description='Edit xml files from data collection.')
parser.add_argument('batch_name', nargs=1, help='Add a csv file.!')
args = parser.parse_args()

if args.batch_name:
    batchName = args.batch_name[0].strip()
elif args.csv_file is None:
    print "no folder name"

folderName = "./" + batchName + "/"
listofFiles = folderName + batchName + ".csv"
data = folderName + batchName + ".xml"
...
with open(listofFiles, mode='rU') as f:
...
output_file = folderName + os.path.splitext(os.path.basename(data))[0] + "_edited.xml"



""" delete previous version of edited xml file"""
...
for x in root.findall("./Prompt/timeKeeping/promptStartMillis"):
